...
from jax.experimental.colocated_python import serialization as colocated_serialization
...
def _colocated_deserialize_setup(config_bytes: jax.Array) -> jax.Array:
    """Initialize TensorStore context and event loop on colocated Python workers.

    Must be called before each deserialization to ensure fresh resources,
    avoiding accumulation of stale resources across checkpoint reloads.
    """
    print("In colocated deserialize setup")
    colocated_python.resource_manager = _ColocatedResourceManager()
    config_bytes_np = np.asarray(config_bytes).tobytes()
    config = colocated_serialization._deserialize(config_bytes_np)
    colocated_python.resource_manager.current_config = config
    print("In colocated deserialize setup, all variables setup")
    return config_bytes


@colocated_python.colocated_python
def _load_to_cpu(idx: jax.Array):
    """Module-level function to avoid colocated_python cache busting on every restore."""
    i = int(idx)
    print("In load_to_cpu")
    print("rm in colocated: ", colocated_python.resource_manager)
    rm = colocated_python.resource_manager
    config = rm.current_config
    cpu_shardings = config["cpu_shardings"]
    tensorstore_specs = config["tensorstore_specs"]
    global_shapes = config["global_shapes"]
    dtypes = config["dtypes"]
    concurrent_bytes = config["concurrent_bytes"]
...
async def _run_colocated_deserialize(
    ...
    *,
    ...
    # Initialize fresh TensorStore context and event loop on the sidecar workers.
    config_dict = {
        "cpu_shardings": cpu_shardings,
        "tensorstore_specs": tensorstore_specs,
        "global_shapes": global_shapes,
        "dtypes": dtypes,
        "concurrent_bytes": concurrent_bytes,
    }
    config_bytes_str = colocated_serialization._serialize(config_dict)
    config_bytes_np = np.frombuffer(config_bytes_str, dtype=np.uint8)
    config_bytes_array = jax.device_put(jnp.array(config_bytes_np), replicated_sharding)
    _colocated_deserialize_setup.specialize(devices=cpu_devices)(config_bytes_array)
    ...
    async def _load_and_transfer_one(
        ...
        specialized = _load_to_cpu.specialize(
            devices=cpu_devices,

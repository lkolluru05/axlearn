...
class ArtifactRegistryBundler(DockerBundler):
    ...
    def _build_and_push(self, *args, **kwargs):
        ...
        if bool(cfg.colocated_image_required):
            # docker_bundler_cfg = config_for_class(BaseDockerBundler).set(
            #     image=cfg.colocated_image_name,
            #     repo=cfg.repo,
            #     dockerfile=cfg.colocated_dockerfile
            # )
            # bundler_cfg = config_for_class(ColocatedArtifactRegistryBundler)
            # bundler_cfg.set(cfg=docker_bundler_cfg)
            # docker_bundler_cfg = cfg
